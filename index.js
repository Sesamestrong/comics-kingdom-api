...
const regex = /([^\/]+)\/\d+\/\d+\/([^.]+)\.(\d{4})(\d{2})(\d{2})_(\d+)\.(.+)$/;
...
app.get("/",(req, res) => res.send("This is under construction."));
app.post("/getUrl", (req, res) => {
    ...
    const [_, name, escName, sampleYear, sampleMonth, sampleDay, size, format] = Buffer(base64, "base64").toString().match(regex);
    const undName = isSunday(sampleYear, sampleMonth, sampleDay) ? escName.split("_").slice(0,-1).join("_"):escName;
